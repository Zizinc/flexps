...
DEFINE_double(drop_rate, 0.1, "The drop rate used for training tree");
...
void Run() {
  ...
  */
  ...
  task.SetLambda([& parameter_server, &train_data_loader, &test_data_loader
    ...
    // Set DataLoader
    dart->set_training_dataset(local_train_data_loader);
    dart->set_test_dataset(local_test_data_loader);
