...
class NDCGScorer: public Scorer {
  public:
    NDCGScorer();
    ...
    std::map<std::string, std::vector<float>> calculate_lambda_and_weight_vect(std::vector<float> vect, std::vector<float> score_vect);
    std::map<std::string, std::vector<float>> calculate_tmp_lambda_and_weight_vect(int qid, std::vector<float> vect, std::vector<float> score_vect);
