...
ChannelUtil::ChannelUtil(Node& my_node, std::vector<Node>& nodes) {
  this->my_node = my_node;
  this->nodes = nodes;
}

void ChannelUtil::channel_for_balancing_hdfs_data(DataLoader& train_data_loader, int& global_data_num) {
...
std::vector<std::map<std::string, float>> ChannelUtil::channel_for_global_min_max_feat(std::vector<std::map<std::string, float>>& min_max_feat_list) {
...
void ChannelUtil::channel_for_global_predict_result(const std::map<std::string, float>& local_predict_result) {
