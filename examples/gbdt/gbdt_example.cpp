...
// dataset config
DEFINE_string(data_format, "svm_lib", "The data format of dataset");
...
void Run() {
  ...
  train_data_loader.read_hdfs_to_class_feat_vect(data);
  ...
  */
  ...
  LOG(INFO) << "DataLoader load train data successfully";
  ...
  //std::vector<std::map<std::string, float>> min_max_feat_list = & _min_max_feat_list;
  ...
  for (std::map<std::string, float> map: global_min_max_feat_list) {
    min_max_string += std::to_string(map["min"]) + "," + std::to_string(map["max"]) + " ";
  }
  LOG(INFO) << "min_max_string = " << min_max_string;
  
  
  // Load testing set
  DataLoader test_data_loader(FLAGS_local_test_input);
  std::vector<std::vector<float>> all_test_feat_vect_list = test_data_loader.get_feat_vect_list();
  std::vector<float> all_test_class_vect = test_data_loader.get_class_vect();
  ...
  task.SetLambda([nodeId, worker_num, & all_predict_result
    ...
    */
    ...
    //auto & table = name_to_kv_table["grad_and_hess"];
    //auto table1 = info.CreateKVClientTable<float>(kTableId);
    //auto & table = table1;
